...
def fingerprinters():
    fingerprint_columns = ["canvas", "canvas_font"]
    fingerprint_rows = []
    fingerprint_data = [0,0]
    if data["reports"]["canvas_fingerprinters"]["data_url"]:
        fingerprint_data[0] = 1
    if data["reports"]["canvas_font_fingerprinters"]["canvas_font"]:
        fingerprint_data[1] = 1
    fingerprint_rows.append(fingerprint_data)
    fingerprint_df = pd.DataFrame(fingerprint_rows, columns=fingerprint_columns)
    fingerprint_df.to_csv("fingerprinters.csv", mode="a", header=False, index=False)
...
def scripts():
    ...
    if data["reports"]["google_analytics_events"]:
        tp_scripts_data[0] = 1
    if data["reports"]["fb_pixel_events"]:
        tp_scripts_data[1] = 1
    for tp_hosts in data["hosts"]["requests"]["third_party"]:
...
def cookies():
    ...
    for cookie in data["reports"]["cookies"]:
        ...
        if cookie.get("third_party") == False:
            fp_cookies += 1
